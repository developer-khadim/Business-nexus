...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    login(email, password, role)
      .then((response) => {
        ...
        navigate(response.user?.role === "entrepreneur"
  ...
  const handleDemoLogin = (userRole: UserRole) => {
    ...
    login(demoEmail, demoPassword, userRole)
      .then((response) => {
        navigate(response.user?.role === "entrepreneur"
          ? "/dashboard/entrepreneur"
          : "/dashboard/investor"
      ...
      .catch((err) => {
        // Fallback: If login fails (maybe seed didn't run?), try filling credentials at least
        console.error("Demo login failed:", err);
        setError("Demo login failed. Data might not be seeded. Refreshing...");
        setEmail(demoEmail);
        setPassword(demoPassword);
        setRole(userRole);
        // Force reload if needed or just show error
      })
      .finally(() => setIsLoading(false));
  };
