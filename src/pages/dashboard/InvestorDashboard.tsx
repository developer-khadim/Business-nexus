...
export const InvestorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      const res = await getAllEntrepreneurs();
      if (res.success) setEntrepreneurs(res.entrepreneurs);
    })();
  }, []);
  ...
  // Filter entrepreneurs based on search and industry filters
    ...
    const { name, startupName, industry, pitchSummary } = entrepreneur;

    // safely lowercase values (fallback to empty string if null)
    const matchesSearch =
      searchQuery === "" ||
  ...
  // Get unique industries for filter (skip null/empty)
  const industries = Array.from(
    new Set(
      entrepreneurs
        .map((e) => e.industry)
        .filter((ind): ind is string => ind != null && ind.trim() !== "")
    )
  );
          ...
          <CardBody>
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-xl font-semibold text-primary-900">{dashboard.totalStartups}</h3>
              </div>
            </div>
          </CardBody>
        </Card>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-xl font-semibold text-accent-900">
                  {dashboard.collaborations.filter(req => req.status === 'accepted').length}
                </h3>
              </div>
            </div>
          </CardBody>
